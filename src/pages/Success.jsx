    ...
    const fetchSessionDetails = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/stripe/api/session/${session_id}`
        );
        if (!response.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto my-40 p-5 bg-green-100 rounded-lg shadow">
      ...
      {orderDetails ? (
        <div>
          <p className="mt-4">
            Thank you, {orderDetails.customerEmail}, for your purchase. Here are
            your order details:{" "}
          </p>
          <ul className="mt-4">
            {orderDetails.items.map((item, index) => (
              <li key={index}>
                Order: {item.name}, quantity: {item.quantity}
              </li>
            ))}
          </ul>
      ...
      <div className="mt-4">
        <Link
          to="/"
          className="bg-green-600 hover:bg-green-900 text-white py-2 px-4 font-semibold"
        >
          Go Back to Homepage
        </Link>
      </div>
